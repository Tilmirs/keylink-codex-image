...
11. For image editing, pass exactly one of `--input-image-path` or `--input-image-url`. The helper uploads the actual image bytes under the `image` multipart field, adds an edit instruction, preserves unspecified content, returns the edited image, and labels the result `Operation: edit`. Do not send `input_image` JSON to Keylink's `/v1/images/edits`; it is not recognized as a file. If the automatic endpoint retry is used, report the effective endpoint, the `AttemptOrder`, and the failed attempt details while preserving the original prompt/reference image.
