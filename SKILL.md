...
8. Apply the resolution policy before invoking the helper. For GPT Plus `gpt-image-2`, the conservative default choices are `1024x1024`, `1536x1024`, and `1024x1536`; choose the one matching the user's requested composition. Do not assume `2048x2048` when the service has not advertised it. Treat “更高分辨率”, “高分辨率”, “高清”, “超高清”, “2K”, “4K”, “UHD”, or an explicit larger pixel size as high-resolution intent. For a 16:9 request, try `2560x1440` for 2K-class output and `3840x2160` for 4K output. Preserve the user-selected model ID, including `gpt-image-2`, for every resolution tier.
9. If a requested 4K size is not advertised, show the actual advertised sizes and ask whether the user approves local upscaling before doing any local resize. If a high-resolution request is sent to the service and it fails, preserve the original status/body error, explain that the model, channel, or requested size rejected high resolution, and do not silently retry at 1K or switch to Chat. A high-resolution edit must keep the original/reference image and use Images Edits when available.
10. Use `--route auto` unless the user explicitly selects a route. When the endpoint is omitted, known image models prefer Keylink Images: text-only requests use `/v1/images/generations`, while reference-image edits first use `/v1/images/edits` with a multipart `image` file field. If that implicit Images Edits request returns an endpoint/capability error (HTTP 400/404/405/415/422/501 with an unsupported-edit or multipart indication), retry the same edit once through `/v1/chat/completions`; do not retry for authentication, rate-limit, content, network, or server errors. An explicit `--endpoint-mode images`, explicit `--endpoint`, or explicit `--endpoint-mode chat` disables this automatic endpoint fallback.
11. For image editing, pass exactly one of `--input-image-path` or `--input-image-url`. The helper uploads the actual image bytes under the `image` multipart field, adds an edit instruction, preserves unspecified content, returns the edited image, and labels the result `Operation: edit`. Do not send `input_image` JSON to Keylink's `/v1/images/edits`; it is not recognized as a file. If the automatic Images-to-Chat fallback is used, report the effective Chat endpoint and preserve the original prompt/reference image.
...
- GPT Plus `gpt-image-2` default: use the conservative sizes `1024x1024`, `1536x1024`, or `1024x1536` according to the requested composition. These are the safe defaults when the catalog has no size metadata.
