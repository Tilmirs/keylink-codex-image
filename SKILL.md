...
8. Apply the resolution policy before invoking the helper. For `gpt-image-2` and the supported Gemini image models, the conservative default choices are `1024x1024`, `1536x1024`, and `1024x1536`; choose the one matching the user's requested composition. Do not assume `2048x2048` when the service has not advertised it. Treat “更高分辨率”, “高分辨率”, “高清”, “超高清”, “2K”, “4K”, “UHD”, or an explicit larger pixel size as high-resolution intent. For a 16:9 request, try `2560x1440` for 2K-class output and `3840x2160` for 4K output. Preserve the user-selected model ID for every resolution tier.
